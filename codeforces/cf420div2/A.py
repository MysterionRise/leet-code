...
class SegmentTree:
    def __init__(self, values: List[int], commutative: Callable[[int, int], int], zero: int):
        self.n = len(values)
...
def solve():
    n = nextInt()
    arr = [[nextInt() for _ in range(n)] for _ in range(n)]
    ...
    for i in range(n):
        for j in range(n):
            if arr[i][j] != 1:
                ...
                for k in range(n):
                    for l in range(n):
                        if k != j and l != i and arr[i][k] + arr[l][j] == arr[i][j]:
                            flag = True
                            break
                    if flag:
                        break
